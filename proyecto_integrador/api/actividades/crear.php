...
if (empty($idMateria) || empty($titulo) || empty($fechaLimite) || empty($ponderacion)) {
    respuestaJSON(false, null, 'Todos los campos obligatorios deben completarse', 400);
}
...
if ($fechaLimiteTimestamp <= time()) {
    respuestaJSON(false, null, 'La fecha límite debe ser futura', 400);
}
...
try {
    ...
    $stmt = $db->prepare("SELECT id_materia FROM materias WHERE id_materia = ? AND id_maestro = ? AND activa = 1");
    $stmt->execute([$idMateria, $idMaestro]);
    $materia = $stmt->fetch();
    
    if (!$materia) {
        respuestaJSON(false, null, 'Materia no válida o no tienes permisos', 400);
    }
    ...
    if ($stmt->execute([$idMateria, $titulo, $descripcion, $fechaLimite, $ponderacion, $tipo])) {
        ...
        respuestaJSON(false, null, 'Error al crear la actividad en la base de datos', 500);
    }
    
} catch (PDOException $e) {
    error_log("Error al crear actividad: " . $e->getMessage());
    respuestaJSON(false, null, 'Error del servidor: ' . $e->getMessage(), 500);
}
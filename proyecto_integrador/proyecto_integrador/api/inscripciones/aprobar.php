...
try {
    ...
    if ($stmt->execute([$idInscripcion])) {
        respuestaJSON(true, ['id_inscripcion' => $idInscripcion], 'Inscripción aprobada exitosamente');
    } else {
        respuestaJSON(false, null, 'Error al aprobar la inscripción', 500);
    }
